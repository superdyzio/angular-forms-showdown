...
22 slide components live in `src/app/slides/`. The same complex user-registration form (dynamic addresses, async email validation, password strength, conditional fields) is implemented three times to make comparisons fair.
...
| `src/app/lang/en.json` | 305 translation keys (source of truth for copy; `pl.json` mirrors it exactly) |
...
- ~~Tests are minimal~~ — 9 spec files, **161 tests** (validators, services, and all three form components)
